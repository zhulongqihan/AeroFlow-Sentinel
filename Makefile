# AeroFlow Sentinel Makefile
# 用于自动化航旅预订链路稳定性项目初始化和知识文档向量化
...
# 默认目标：显示帮助信息
help:
	@echo "$(GREEN)AeroFlow Sentinel Makefile$(NC)"
	@echo ""
	@echo "可用命令："
	...
	@echo "  $(YELLOW)make upload$(NC)  - 上传 aiops-docs 目录下的所有航旅知识文档"
...
init:
	@echo "$(GREEN)🚀 开始一键初始化 AeroFlow Sentinel...$(NC)"
	@echo ""
	...
	@$(MAKE) wait
	@echo ""
	@echo "$(YELLOW)步骤 4/4: 上传航旅链路知识文档到向量数据库$(NC)"
	@$(MAKE) upload
	@echo ""
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
...
test-upload:
	...
	@if [ -f "$(DOCS_DIR)/checkout_latency_spike.md" ]; then \
		curl -X POST $(UPLOAD_API) \
			-F "file=@$(DOCS_DIR)/checkout_latency_spike.md" \
