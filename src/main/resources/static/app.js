// AeroFlow Sentinel 前端应用
class AeroFlowSentinelApp {
    constructor() {
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/flight_guard`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                }
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        // 流结束，更新最终内容
                        if (fullResponse) {
                            console.log('航旅链路巡检流结束，更新最终内容，长度:', fullResponse.length);
                    ...
                    for (const line of lines) {
                        ...
                        console.log('[Flight Guard SSE] 收到行:', line);
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            currentEvent = line.substring(6).trim();
                            console.log('[Flight Guard SSE] 事件类型:', currentEvent);
                            continue;
                        } else if (line.startsWith('data:')) {
                            const rawData = line.substring(5).trim();
                            console.log('[Flight Guard SSE] 数据:', rawData, ', currentEvent:', currentEvent);
                            ...
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    console.log('[Flight Guard SSE] 匹配到', matches.length, '个JSON对象');
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                console.log('航旅链路巡检流完成，最终内容长度:', fullResponse.length);
                                                this.updateAIOpsMessage(loadingMessageElement, fullResponse, []);
                                                return true;
                                            } else if (sseMessage.type === 'error') {
                                                throw new Error(sseMessage.data || '航旅链路巡检分析失败');
                                            }
                                        } catch (e) {
                                            if (e.message.includes('航旅链路巡检')) throw e;
                                            console.log('[Flight Guard SSE] 单个JSON解析失败:', jsonStr);
                                        }
                                    }
    ...
    async triggerAIOps() {
        ...
        try {
            ...
            console.error('航旅链路巡检分析失败:', error);
            // 更新消息为错误信息
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，航旅链路巡检分析时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '航旅链路巡检分析中，请稍候...';
                if (loadingSubtext) loadingSubtext.textContent = '正在分析预订链路风险，请耐心等待';
                // 防止页面滚动
