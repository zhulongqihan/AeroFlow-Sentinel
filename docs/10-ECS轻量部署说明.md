...
这份文档解释项目如何在 2C2G 等低配 ECS 环境中运行，以及轻量部署模式的适用边界。
...
轻量部署模式的核心价值，是让 AeroFlow Sentinel 在低资源机器上也能稳定运行关键链路，同时明确依赖降级和生产边界。
